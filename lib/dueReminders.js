...
const DZIKIR_PAGI_MIN = 5 * 60 + 15;   // 05:15
const DZIKIR_SORE_MIN = 16 * 60;  // 16:00
const FINANCE_MIN     = 22 * 60;       // 22:00
...
async function getShalatSchedule(provinsi, kabkota, bulan, tahun, cache, trace) {
  ...
  try {
    ...
    if (trace) trace.shalatApiResponse = { httpStatus: res.status, code: json.code, message: json.message };
  } catch (e) {
    if (trace) trace.shalatApiError = e.message;
  }
  cache.set(key, data);
  return data;
}
...
// trace (opsional): object yang diisi info diagnostik untuk keperluan debug
async function computeDueNotifications(sub, db, scheduleCache, trace) {
  const notifs = [];
  const { nowMin, offset, m, y, d, dateStr } = localDateParts(sub.provinsi);
  if (trace) Object.assign(trace, { nowMin, m, y, d, dateStr, offset });
  ...
  if (sub.provinsi && sub.kabkota) {
    const jadwalBulan = await getShalatSchedule(sub.provinsi, sub.kabkota, m, y, scheduleCache, trace);
    const todayJadwal = jadwalBulan && jadwalBulan.jadwal && jadwalBulan.jadwal.find((j) => j.tanggal === d);
    if (trace) {
      trace.jadwalFetched = !!jadwalBulan;
      trace.jadwalCode = jadwalBulan ? 'ok' : 'null';
      trace.todayJadwal = todayJadwal || null;
    }
    if (todayJadwal) {
